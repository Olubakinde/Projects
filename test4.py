...
class AttendanceTaker:
    ...
    def swipe_card(self, student_id, swipe_time=None):
        print(f"Swipe time: {swipe_time}, Class start time: {self.class_start_time}, Class end time: {self.class_end_time}")
        ...
        if not self.student_attendance[student_id]["attendance"]:
            ...
            last_status = self.student_attendance[student_id]["attendance"][-1]["status"]
            attendance_status = "Present" if last_status == "Absent" else "Absent"
    ...
    def send_present_notification(self, student_id):
        sender_email = "olubakindetito@gmail.com"
        receiver_email = "tolubaki@udel.edu"
        password = "cxfg alju sfta mnha"
        ...
        message["From"] = sender_email
        message["To"] = receiver_email
        message["Subject"] = "Present Notification"

        current_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        body = f"Dear Student,\n\nYour attendance has been recorded as present for the class on {current_date}.\n\nRegards,\nAttendance System"
        message.attach(MIMEText(body, "plain"))
        ...
        try:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                server.login(sender_email, password)
                server.sendmail(sender_email, receiver_email, message.as_string())
            print("Present notification sent successfully.")
        except Exception as e:
            print(f"Failed to send present notification: {e}")
    ...
    def send_early_swipe_notification(self, student_id):
        ...
        current_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        body = f"Dear Student,\n\nYou have swiped your card early for the class on {current_date}. Please ensure to swipe your card within the specified time frame.\n\nRegards,\nAttendance System"
        message.attach(MIMEText(body, "plain"))
    ...
    def check_for_early_swipe(self, student_id):
        if student_id in self.student_attendance:
            last_swipe_time = self.student_attendance[student_id]["attendance"][-1]["time"]
            if last_swipe_time < self.class_start_time:
                self.send_early_swipe_notification(student_id)

    def check_for_late_swipe(self, student_id):
        if student_id in self.student_attendance:
            last_swipe_time = self.student_attendance[student_id]["attendance"][-1]["time"]
            if last_swipe_time > self.class_end_time:
                self.send_late_swipe_notification(student_id)

    def check_for_early_leave(self, student_id):
        if student_id in self.student_attendance:
            last_swipe_time = self.student_attendance[student_id]["attendance"][-1]["time"]
            if self.class_end_time - last_swipe_time > timedelta(minutes=30):
...
if __name__ == "__main__":
    ...
    # Check for early swipes and send notifications
    attendance_taker.check_for_early_swipe("123456789")
